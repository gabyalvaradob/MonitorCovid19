...
const loadButtons = (data) => {
    ...
    active_number.innerText = numeral(`${data.active}`).format('0a').toUpperCase();
    ...
    todaysActives = document.querySelector('.todays-actives')
    ...
    todaysActives.innerHTML = numeral(data.todayCases).format('+0,0')
    ...
    data.map((country) => {
        ...
        var html = `
            <div class="info-container">
                <div class="info-flag" style="background-image: url(${country.countryInfo.flag});background-position: center;">
                </div>
                <div class="info-name">
                    ${country.country}
                </div>
                ...
                <div class="info-confirmed">
                    Activos: ${thousands_separators(country.active)}
                </div>
                <div class="info-recovered">
                    Recuperados: ${thousands_separators(country.recovered)}
                </div>
                <div class="info-deaths">   
                    Muertes: ${thousands_separators(country.deaths)}
                </div>
            </div>
        `
...
const showDataInTable = (data) => {
    ...
    data.forEach((country) => {
        html += `
        <tr>
            <td class="first-td hide"><img class="flag-table" src= ${country.countryInfo.flag}></td>
...
displayNewTable = (arr, col) => {
    ...
    for (i = 0; i < arr.length - 1; i++) {
        dataInfoadd = `
    <tr>
            <td class="first-td hide"><img class="flag-table" src= ${arr[i].flag}></td>
...
const tablelisterer = () => {
    ...
    for (i = 0; i < tr.length; i++) {
        ...
        tr[i].addEventListener('click', function () {
            country = (this.childNodes[1].nextSibling.parentElement.cells[1].childNodes[0].data)
...
const refresh = () => {
    ...
    map.setCenter({ lat: 20.97537, lng: -89.61696 })
    map.setZoom(2)
    filtertable()
}
